...
(function () {
    ...
    const ICONS = {
        ...
        stageactive: ICON_BASE + "swatches-gold.png",
        stagebefore: ICON_BASE + "swatch-gold.png",
    };
...
function wireUIOnce() {
    ...
    // Make sure the icon starts closed, and hearts reflect saved stage immediately
    updateStageIcon(false);
    setActiveStageButton(stageKey);
    ...
    if (els.btnStage && els.stageSubmenu) {
        els.btnStage.onclick = () => {
            ...
            const willOpen = els.stageSubmenu.hidden; // if hidden now, we're about to open
    ...
    // Handle submenu selection (delegated)
